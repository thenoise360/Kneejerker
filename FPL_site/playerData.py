...
def pointsPerSelectedPercentage(minimumPercentageSelected, maximumCost, minimumCost):
    ...
    for gameweekData in currentDumps['elements']:
        ...
        genericMethods.runPercentage(length, currentIndex, "Generating points per percentage selected dictionary", f"Dictionary created for all players up to {maximumCost}M pounds values and selected by {minimumPercentageSelected}%")
        tempDict = dict()
        playerID = gameweekData['id']
